...
//TODO	Inizializzare il timer in FreeRTOS
//TODO 	Provare se funziona senza OS
//TODO	Deallocare le risorse del sistema operativo sulla deinit
//TODO 	Scrivere un solo LOG di Maintence alla pressione del bottone ed evitare scritture multiple
...
EventLogger_Status_TypeDef EventLogger_Init(EventLogger_Handle_TypeDef *plog, EventLogger_Interface_TypeDef *ploginterface){
    ...
    osThreadDef(LogCollector, LogCollector_Process_OS, osPriorityLow, 0, (8 * configMINIMAL_STACK_SIZE));
...
EventLogger_Status_TypeDef EventLogger_DeInit(EventLogger_Handle_TypeDef *plog){
	...
	//Deinizzializzare le strutture del sistema operativo
...
EventLogger_Status_TypeDef EventLogger_LogEvent(EventLogger_Handle_TypeDef *plog, uint Facility,
	...
	strcat(TempLog.LogString,IntegerToString(_sys_time.year,String));
	strcat(TempLog.LogString, ":");
	strcat(TempLog.LogString,IntegerToString(_sys_time.month,String));
	strcat(TempLog.LogString, ":");
	strcat(TempLog.LogString,IntegerToString(_sys_time.day,String));
	strcat(TempLog.LogString, ",");
	strcat(TempLog.LogString,IntegerToString(_sys_time.hour,String));
	strcat(TempLog.LogString, ":");
	strcat(TempLog.LogString,IntegerToString(_sys_time.minutes,String));
	strcat(TempLog.LogString, ":");
	strcat(TempLog.LogString,IntegerToString(_sys_time.seconds,String));
	strcat(TempLog.LogString, ",");
	...
	do{
		switch(Next){
			case LOG_INT:
				...
				strcat(TempLog.LogString, "=");
				//Int_Value = va_arg(Description_Params, int);
	...
	LogIndex_List[i] = 1;
	strcpy(Log_List[i].LogString, TempLog.LogString);
	osMessagePut ( plog->os_event, i, 0);
#else	//ELSE BUFFERING
	strcpy(plog->pLogData->LogString, TempLog.LogString);
