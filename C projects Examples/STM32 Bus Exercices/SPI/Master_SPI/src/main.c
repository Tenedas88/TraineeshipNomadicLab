 ...
 *
 * @brief	Questo Main gestisce un componente SPI Master che invia.
 */
#include <stdio.h>
...
SPI_HandleTypeDef SpiMasterHandle;
  ...
  while (KeepLoop)
    {
		...
		while((Debug =HAL_SPI_Transmit(&SpiMasterHandle, (uint8_t*)txMasterBuffer, TXBUFFERSIZE, 5000))!=HAL_OK){
		}
